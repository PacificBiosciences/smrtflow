...
import db.migration.sqlite.V15Data
...
class V16__WriteDataToH2 extends JdbcMigration with SlickMigration {
  override def slickMigrate(db: DatabaseDef): Future[Any] = {
    require(V15Data.data.isDefined, "V15__ReadDataForH2 did not run successfully")

    val data = V15Data.data.get
    db.run {
      V16Schema.allTables.map(_.schema).reduce(_ ++ _).create >>
        (V16Schema.engineJobs ++= data.engineJobs) >>
        (V16Schema.jobTags ++= data.jobTags) >>
        (V16Schema.projects ++= data.projects) >>
        (V16Schema.jobEvents ++= data.jobEvents) >>
        (V16Schema.jobsTags ++= data.jobsTags) >>
        (V16Schema.projectsUsers ++= data.projectsUsers) >>
        (V16Schema.datasetTypes ++= data.datasetTypes) >>
        (V16Schema.engineJobsDataSets ++= data.engineJobsDataSets) >>
        (V16Schema.dsMetaData2 ++= data.dsMetaData2) >>
        (V16Schema.dsSubread2 ++= data.dsSubread2) >>
        (V16Schema.dsHdfSubread2 ++= data.dsHdfSubread2) >>
        (V16Schema.dsReference2 ++= data.dsReference2) >>
        (V16Schema.dsGmapReference2 ++= data.dsGmapReference2) >>
        (V16Schema.dsAlignment2 ++= data.dsAlignment2) >>
        (V16Schema.dsBarcode2 ++= data.dsBarcode2) >>
        (V16Schema.dsCCSread2 ++= data.dsCCSread2) >>
        (V16Schema.dsCCSAlignment2 ++= data.dsCCSAlignment2) >>
        (V16Schema.dsContig2 ++= data.dsContig2) >>
        (V16Schema.datastoreServiceFiles ++= data.datastoreServiceFiles) >>
        (V16Schema.runSummaries ++= data.runSummaries) >>
        (V16Schema.dataModels ++= data.dataModels) >>
        (V16Schema.collectionMetadata ++= data.collectionMetadata) >>
        (V16Schema.samples ++= data.samples)
    }
  }
}
