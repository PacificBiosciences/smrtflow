...
class V15__ReadDataForH2 extends JdbcMigration with SlickMigration {
  override def slickMigrate(db: DatabaseDef): Future[Any] = {
    val action = for {
      je   <- V15Schema.jobEvents.result
      jt   <- V15Schema.jobTags.result
      jst  <- V15Schema.jobsTags.result
      ej   <- V15Schema.engineJobs.result
      p    <- V15Schema.projects.result
      pu   <- V15Schema.projectsUsers.result
      dst  <- V15Schema.datasetTypes.result
      ejds <- V15Schema.engineJobsDataSets.result
      dsm  <- V15Schema.dsMetaData2.result
      sds  <- V15Schema.dsSubread2.result
      hds  <- V15Schema.dsHdfSubread2.result
      rds  <- V15Schema.dsReference2.result
      gds  <- V15Schema.dsGmapReference2.result
      ads  <- V15Schema.dsAlignment2.result
      bds  <- V15Schema.dsBarcode2.result
      cds  <- V15Schema.dsCCSread2.result
      cads <- V15Schema.dsCCSAlignment2.result
      cods <- V15Schema.dsContig2.result
      dsf  <- V15Schema.datastoreServiceFiles.result
      rs   <- V15Schema.runSummaries.result
      dm   <- V15Schema.dataModels.result
      cm   <- V15Schema.collectionMetadata.result
      s    <- V15Schema.samples.result
    } yield new V15Data(je, jt, jst, ej, p, pu, dst, ejds, dsm, sds, hds, rds, gds, ads, bds, cds, cads, cods, dsf, rs, dm, cm, s)

    db.run(action).map(d => V15Data.data = Some(d))
  }
}

object V15Data {
  var data: Option[V15Data] = None
}
