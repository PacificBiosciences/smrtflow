...
object LoggerOptions {
  ...
  private def validateLogLevel(level: String): Either[String, Unit] = {
    if (VALID_LOG_LEVELS contains level.toUpperCase) Right(Unit)
      ...
      Left(
        s"Invalid log level '${level.toUpperCase}'. Valid log levels: ${VALID_LOG_LEVELS
          .reduce(_ + "," + _)}")
  }
  ...
  def add(parser: OptionParser[LoggerConfig]): Unit = {
    ...
    parser
      .opt[File]("logback")
      .action { (x, c) =>
        c.configure(x.toPath.toAbsolutePath.toString,
                    c.logFile,
                    c.debug,
                    c.logLevel)
      }
  ...
  def parse(args: Seq[String],
            lc: LoggerConfig = new LoggerConfig() {}): Unit = {
  ...
  def parseAddDebug(args: Seq[String]): Unit = {
    ...
    val v =
      if (args.filter(requireOne).isEmpty) args :+ "--log2stdout" else args
    parse(v)
  }
}
