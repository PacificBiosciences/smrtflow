...
/**
  ...
  */
class ConvertImportFastaJob(opts: ConvertImportFastaOptions)
    extends BaseCoreJob(opts: ConvertImportFastaOptions)
    with MockJobUtils
    with timeUtils {
        ...
        x match {
          case Right(referenceDatasetFileIO) =>
            ...
            val dsFile = toDataStoreFile(
              UUID.fromString(referenceDatasetFileIO.dataset.getUniqueId),
              referenceDatasetFileIO.path)
            val datastore =
