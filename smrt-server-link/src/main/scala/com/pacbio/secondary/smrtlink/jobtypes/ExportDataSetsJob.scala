...
/**
  ...
  */
case class ExportDataSetsJobOptions(datasetType: DataSetMetaTypes.DataSetMetaType,
                                    ids: Seq[IdAble],
                                    outputPath: Path,
                                    name: Option[String],
  ...
  override def resolveEntryPoints(dao: JobsDao): Seq[EngineJobEntryPointRecord] = {
    val fx = for {
      datasets <- ValidateServiceDataSetUtils.resolveInputs(datasetType, ids, dao)
