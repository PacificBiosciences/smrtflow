...
case class DatabaseToolOptions(username: String,
                               ...
                               server: String,
                               port: Int = 5432,
                               mode: DbModes.Mode = DbModes.UNKNOWN)
    extends LoggerConfig

object DatabaseTool
    extends CommandLineToolRunner[DatabaseToolOptions]
  ...
  val defaults =
    DatabaseToolOptions("smrtlink_user", "password", "smrtlink", "localhost")
  ...
  val parser = new OptionParser[DatabaseToolOptions]("database-tools") {
    ...
    opt[String]('u', "user")
      ...
      .text(s"Database user name ${toDefault(defaults.username)}")
    opt[String]('p', "password")
      .action { (x, c) =>
        c.copy(password = x)
      }
      .text(s"Database Password ${toDefault(defaults.password)}")
    opt[String]('s', "server")
      .action { (x, c) =>
        c.copy(server = x)
      }
      .text(s"Database server ${toDefault(defaults.server)}")
    opt[String]('n', "db-name")
      .action { (x, c) =>
        c.copy(dbName = x)
      }
      .text(s"Database Name ${toDefault(defaults.dbName)}")
    opt[Int]("port")
      .action { (x, c) =>
        c.copy(port = x)
      }
      .text(s"Database port ${toDefault(defaults.port.toString)}")
