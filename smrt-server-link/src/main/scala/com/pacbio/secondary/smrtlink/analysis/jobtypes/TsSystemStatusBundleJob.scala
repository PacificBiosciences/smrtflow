...
case class TsSystemStatusBundleOptions(smrtLinkSystemRoot: Path,
...
class TsSystemStatusBundleJob(opts: TsSystemStatusBundleOptions)
  ...
  def run(job: JobResourceBase,
    ...
    TechSupportUtils.writeSmrtLinkSystemStatusTgz(
      opts.manifest.smrtLinkSystemId,
      smrtLinkUserData,
      outputTgz,
      opts.manifest.user,
    ...
    val dsFile = DataStoreFile(
      ...
      s"Tech Support System Status TGZ bundle for SL System Root ${opts.smrtLinkSystemRoot}"
    )
