...
case class ClusterTemplate(startTemplate: String, stopTemplate: String) {
  ...
  def renderStart(job: CommandTemplateJob) =
    RenderCommandTemplate.renderCommandTemplate(startTemplate, job)
  def renderStop(job: CommandTemplateJob) =
    RenderCommandTemplate.renderCommandTemplate(stopTemplate, job)
...
object ClusterTemplate {
  ...
  def apply(path: Path): ClusterTemplate = {
    ClusterTemplate(loadFrom(path.resolve(START)),
                    loadFrom(path.resolve(STOP)))
  }

  def loadExampleClusterTemplate: ClusterTemplate = {

    val startStream = getClass.getResourceAsStream(
      "/example-command-cluster_templates/sge_pacbio/start.tmpl")
    val stopStream = getClass.getResourceAsStream(
      "/example-command-cluster_templates/sge_pacbio/stop.tmpl")
