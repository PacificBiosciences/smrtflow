...
object ValidateDataSetRunner extends CommandLineToolRunner[ValidatorConfig] {
  ...
  /**
    ...
    * @return
    */
  private def peekIntoXML(
      path: String): Option[DataSetMetaTypes.DataSetMetaType] = {
    Try {
  ...
  def run(config: ValidatorConfig): Either[ToolFailure, ToolSuccess] = {
    ...
    val errorMesssage =
      s"Unsupported dataset type id '$dsTypeError' Supported types $supportedDataSetTypes"
    // For external resources that we defined relative to the XML file
    val rootDir = p.getParent

    def toResult[T <: DataSetType](px: Path, result: Either[String, T]) =
      result match {
        case Right(x) =>
          Right(
            ToolSuccess(s"Successfully validated ${px.toAbsolutePath}",
                        computeTimeDeltaFromNow(startedAt)))
        case Left(ex) =>
          Left(ToolFailure(toolId, computeTimeDeltaFromNow(startedAt), ex))
      }

    def vx[T <: DataSetType](loaderF: (Path => T),
                             validatorF: ((T, Path) => Either[String, T]))
      : (Path => Either[ToolFailure, ToolSuccess]) = { px =>
    ...
    val validateReferenceSet = vx[ReferenceSet](
      DataSetLoader.loadAndResolveReferenceSet,
      DataSetValidator.validate)
    val validateSubreadSet = vx[SubreadSet](
      DataSetLoader.loadAndResolveSubreadSet,
      DataSetValidator.validate)
    val validateHdfSubreadSet = vx[HdfSubreadSet](
      DataSetLoader.loadAndResolveHdfSubreadSet,
      DataSetValidator.validate)
    val validateAlignmentSet = vx[AlignmentSet](DataSetLoader.loadAlignmentSet,
                                                DataSetValidator.validate)
    val validateCCSReadSet = vx[ConsensusReadSet](
      DataSetLoader.loadConsensusReadSet,
      DataSetValidator.validate)
    val validateBarcodeSet =
      vx[BarcodeSet](DataSetLoader.loadBarcodeSet, DataSetValidator.validate)
    val validateConsensusAlignmentSet = vx[ConsensusAlignmentSet](
      DataSetLoader.loadConsensusAlignmentSet,
      DataSetValidator.validate)
    val validateContigSet =
      vx[ContigSet](DataSetLoader.loadContigSet, DataSetValidator.validate)
      ...
      case Some(metatype) =>
        ...
        metatype match {
          ...
          case DataSetMetaTypes.AlignmentCCS =>
            validateConsensusAlignmentSet(p)
