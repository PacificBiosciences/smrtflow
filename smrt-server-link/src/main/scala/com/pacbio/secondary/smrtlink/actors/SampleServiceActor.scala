...
class SampleServiceActor(sampleDao: SampleDao) extends PacBioActor {
  ...
  def receive: Receive = {
    ...
    case CreateSample(login, create) =>
      pipeWith(sampleDao.createSample(login, create))
    case UpdateSample(uniqueId, update) =>
      pipeWith(sampleDao.updateSample(uniqueId, update))
    ...
    Singleton(
      () =>
        actorRefFactory().actorOf(
          Props(classOf[SampleServiceActor], sampleDao()),
          "SampleServiceActor"))
}
...
/**
  ...
  */
trait SampleServiceActorProvider { this: SampleDaoProvider =>
