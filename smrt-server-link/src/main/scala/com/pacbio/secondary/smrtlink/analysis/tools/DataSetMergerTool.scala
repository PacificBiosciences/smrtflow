...
object DataSetMergerTool extends CommandLineToolRunner[DataSetMergerOptions] {
  ...
  def run(config: DataSetMergerOptions): Either[ToolFailure, ToolSuccess] = {
    ...
    val supportedDataSetTypes =
      DataSetMetaTypes.ALL.map(x => DataSetMetaTypes.typeToIdString(x))

    logger.info(s"Loaded config $config")
    logger.info(
      s"resolved '${config.datasetType}' to type ${DataSetMetaTypes.toDataSetType(
        config.datasetType)}. Supported Types $supportedDataSetTypes")
    ...
    val results = for {
      ...
      ds <- DataSetMetaTypes.toDataSetType(config.datasetType) match {
        case Some(DataSetMetaTypes.Subread) =>
          Try {
            DataSetMerger.mergeSubreadSetPathsTo(inputPaths, name, outputPath)
          }
        case Some(DataSetMetaTypes.HdfSubread) =>
          Try {
            DataSetMerger.mergeHdfSubreadSetPathsTo(inputPaths,
                                                    name,
                                                    outputPath)
          }
        case Some(DataSetMetaTypes.Alignment) =>
          Try {
            DataSetMerger.mergeAlignmentSetPathsTo(inputPaths,
                                                   name,
                                                   outputPath)
          }
        case _ =>
          Try {
            throw UnSupportDataSetTypeException(
              s"Unsupported dataset type '${config.datasetType}'. Supported types $supportedDataSetTypes")
          }
      }
    } yield ds
    ...
    results match {
      case Success(x) =>
        logger.info(
          s"wrote merged dataset ${config.datasetType} to ${outputPath.toAbsolutePath.toString}")
      ...
      case Failure(ex) =>
        ...
        Left(
          ToolFailure(toolId,
                      ...
                      s"Failed to merge dataset. ${ex.getMessage}"))
    }
  }
}
