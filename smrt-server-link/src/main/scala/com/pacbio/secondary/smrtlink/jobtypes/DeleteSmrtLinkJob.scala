...
case class DeleteSmrtLinkJobOptions(jobId: IdAble,
  ...
  def confirmIsDeletable(dao: JobsDao, jobId: IdAble, force: Boolean = false): Future[EngineJob] = {
    dao.getJobById(jobId).flatMap { job =>
      if (job.isComplete || force) {
        dao.getJobChildrenByJobId(job.id).flatMap { jobs =>
          ...
          else Future.failed(throw new UnprocessableEntityError(s"Can't delete this job because it has active children. Job Ids: ${jobs.map(_.id)}"))
        }
      }
      else {
        Future.failed(throw new UnprocessableEntityError("Can't delete this job because it hasn't completed"))
      }
    }
  }
...
/**
  ...
  */
class DeleteSmrtLinkJob(opts: DeleteSmrtLinkJobOptions) extends ServiceCoreJob(opts) with timeUtils{
  ...
  override def run(resources: JobResourceBase, resultsWriter: JobResultWriter, dao: JobsDao, config: SystemJobConfig): Either[ResultFailed, PacBioDataStore] = {
    ...
    tx match {
      ...
      case Failure(ex) =>
        ...
        val emsg = s"Failed to run files in DeleteSmrtLinkJob $jobId ${ex.getMessage}"
        resultsWriter.writeError(emsg)
        Left(ResultFailed(resources.jobId, jobTypeId.id, emsg, runTime, AnalysisJobStates.FAILED, host))
    }
  }
}
