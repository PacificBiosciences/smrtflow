...
object MimeTypeProperties {
  ...
  def fromFileName(baseName: String): String = {
    mimeTypes.getOrElse(suffix(baseName), DEFAULT)
  }

  def fromFile(file: File): String = {
    val sx = suffix(file.toPath.toAbsolutePath.toString)
    mimeTypes.getOrElse(sx, DEFAULT)
  }

  lazy val mimeTypes: Map[String, String] = Map(
