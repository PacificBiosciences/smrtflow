...
trait PbDataSetTools {
  ...
  lazy val defaults = CustomConfig(Modes.UNKNOWN,
  ...
  lazy val parser = new OptionParser[CustomConfig]("pb-datasets") {
    ...
    cmd(Modes.MERGE.name) action { (_, c) =>
      c.copy(command = (c) => println(s"Running merge with $c"),
             mode = Modes.MERGE)
    } children (
    ...
    cmd(Modes.IMPORT.name) action { (_, c) =>
      c.copy(command = (c) => println(s"Running import with $c"),
             mode = Modes.IMPORT)
    } children (
      arg[File]("path") action { (s, c) =>
        c.copy(path = s)
...
object PbDataSetToolsApp extends App {
  ...
  def loadMetaTypeFrom(path: Path): Option[DataSetMetaType] =
    DataSetMetaTypes.toDataSetType(
      (scala.xml.XML.loadFile(path.toFile) \ "@MetaType").text)
  ...
  def runMergeDataSet(fofn: Path,
                      outputDataSet: Path,
                      debug: Boolean = false): Int = {
  ...
  def runner(args: Array[String]): Unit = {
    ...
    val xs = PbDataSetTools.parser
      .parse(args.toSeq, PbDataSetTools.defaults) map { config =>
      ...
      val exitCode: Int = config.mode match {
        case Modes.VALIDATE =>
          runValidator(ValidOpts(config.path.toPath, strict = false))
        case Modes.INFO => runInfo(InfoOpts(config.path.toPath, debug = false))
        case Modes.MERGE =>
          runMergeDataSet(config.path.toPath,
                          config.mergeDataSetOutput.toPath,
                          debug = false)
        case Modes.IMPORT =>
          runImportDataSet(config.path.toPath,
                           config.host,
                           config.port,
                           debug = false)
        case unknown =>
          throw new Exception(s"Unsupported command type '$unknown'")
      }
      exitCode
    }
