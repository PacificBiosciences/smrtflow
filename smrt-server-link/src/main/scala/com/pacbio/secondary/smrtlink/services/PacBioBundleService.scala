...
class PacBioBundleService(
    daoActor: ActorRef,
    rootBundle: Path,
    externalPollActor: ActorRef,
    eventManagerActor: ActorRef)(implicit val actorSystem: ActorSystem)
  ...
  val manifest = PacBioComponentManifest(
    ...
    "0.1.0",
    "PacBio Service for registering Bundles of config files for extending the SL Core system"
  )
  ...
  def getBundleIOByTypeAndVersion(
      bundleTypeId: String,
    ...
    (daoActor ? GetBundleIOByTypeAndVersion(bundleTypeId, bundleVersion))
      ...
      .flatMap(failIfNone(
        s"Unable to find Bundle Type '$bundleTypeId' and version '$bundleVersion'"))
  ...
  def activateBundle(bundleType: String,
                     version: String): Future[PacBioDataBundleIO] = {
    val errorMessage =
      s"Unable to upgrade bundle $bundleType and version $version"
              ...
              created {
                for {
                  pathBundle <- fromTry[(Path, PacBioDataBundle)](
                    ...
                    Try {
                      downloadAndParseBundle(record.url, tmpRootBundleDir)
                    })
                  validBundle <- fromTry[PacBioDataBundleIO](
                    s"Bundle Already exists.",
                    Try {
                      copyBundleTo(pathBundle._1, pathBundle._2, rootBundle)
                    })
                  addedBundle <- (daoActor ? AddBundleIO(validBundle))
                    .mapTo[PacBioDataBundleIO]
  ...
  val bundleActivateRoute: Route =
    pathPrefix(ROUTE_PREFIX) {
      (path(Segment / Segment / "upgrade") | path(
      ...
      pathEndOrSingleSlash {
        ...
        path(Segment / Segment / "download") { (bundleTypeId, bundleVersion) =>
          pathEndOrSingleSlash {
            get {
              onSuccess(
                getBundleIOByTypeAndVersion(bundleTypeId, bundleVersion)) {
...
trait PacBioBundleServiceProvider {
  ...
  val pacBioBundleService: Singleton[PacBioBundleService] =
    Singleton { () =>
      ...
      new PacBioBundleService(pacBioBundleDaoActor(),
                              pacBioBundleRoot(),
                              externalBundleUpgraderActor(),
                              eventManagerActor())
    }
