...
case class PbsmrtpipeEngineOptions(
  ...
  def toPipelineOptions: Seq[PipelineBaseOption] = {
    ...
    Seq(
      PipelineBooleanOption(PbsmrtpipeConstants.DEBUG_MODE.id, "pbsmrtpipe DEBUG Mode", debugMode, "Enable pbsmrtpipe DEBUG Mode to pbsmrtpipe.log and master.log"),
      PipelineIntOption(PbsmrtpipeConstants.MAX_NPROC.id, "Max Number of Processors per Task", maxNproc, "Max Number of Processors"),
      PipelineStrOption(PbsmrtpipeConstants.PB_TMP_DIR.id, "Tmp directory", tmpDir.toAbsolutePath.toString, "Root location to temp directory"),
      PipelineBooleanOption(PbsmrtpipeConstants.CHUNKED_MODE.id, "Enable chunk mode", chunkedMode, "Enable chunk mode to create chunk.json from registered chunkable tasks"),
      PipelineIntOption(PbsmrtpipeConstants.MAX_TOTAL_NPROC.id, "Max Total Number of processors to use per pipeline execution", maxTotalNproc, "Total Max Number of slots/proc to use"),
      PipelineIntOption(PbsmrtpipeConstants.MAX_NCHUNKS.id, "Max Number of Chunks per Task", maxNChunks, "Max Number of Chunks per task"),
      PipelineBooleanOption(PbsmrtpipeConstants.DISTRIBUTED_MODE.id, "Enable Distributed mode", distributedMode, s"Enable Distributed mode to submit jobs to the cluster resources. ${PbsmrtpipeConstants.CLUSTER_MANAGER_TMPL_DIR.toString} must be set"),
      PipelineBooleanOption(PbsmrtpipeConstants.EXIT_ON_FAILURE.id, "Enable Exit on Failure", exitOnFailure, "Exit on the first failed task."),
      PipelineStrOption(PbsmrtpipeConstants.CLUSTER_MANAGER_TMPL_DIR.id, "Cluster Template Directory", clusterManagerTemplateDir getOrElse "" , "Directory of Cluster templates (must contain start.tmpl and stop.tmpl) or python module path (e.g, pbsmrtpipe.my_module.sge"),
      PipelineIntOption(PbsmrtpipeConstants.MAX_NWORKERS.id, "Max Number of Workers", maxNWorkers, "Max Number of concurrently running tasks. (Note:  max_nproc will restrict the number of workers if max_nworkers * max_nproc > max_total_nproc)")
    )
  }
...
object PbsmrtpipeEngineOptions {
  ...
  def apply(opts: Seq[ServiceTaskOptionBase]):PbsmrtpipeEngineOptions = {
    ...
    def getBy[T](sx: String, defaultValue: T): T =
      Try { mx.get(sx).map(x => x.value.asInstanceOf[T]) getOrElse defaultValue } getOrElse defaultValue
