...
case class ExampleSubParserToolOptions(a: Int, b: Int, mode: String = "UNKNOWN") extends HasModeAndLoggingConfig

class Alpha extends SubParserModeRunner[ExampleSubParserToolOptions] {
  override def validateOpt(opt: ExampleSubParserToolOptions): Try[ExampleSubParserToolOptions] = Try {opt}

  override val name = "alpha"

  override def run(opt: ExampleSubParserToolOptions): Try[String] = Success(s"Completed running alpha with $opt")
}
...
class Beta extends SubParserModeRunner[ExampleSubParserToolOptions] {
  ...
  override def run(opt: ExampleSubParserToolOptions): Try[String] = Success(s"Completed running beta with $opt")
}


object ExampleSubParserToolRunner extends CommandLineSubParserToolRunner[ExampleSubParserToolOptions] {
  ...
  val parser: OptionParser[ExampleSubParserToolOptions] = new OptionParser[ExampleSubParserToolOptions]("example-subparser") {
    head(DESCRIPTION, VERSION)

    // Each Subparser has to be manually added. I don't really see a way
    // to make this type safe
    cmd(alpha.name).action { (_, c) => c.copy(mode = alpha.name)}
        .children(opt[Int]("a-value").required()
            .action { (p, c) => c.copy(a = p)})
        .text("Run Alpha Subparser")

    cmd(beta.name).action {(_, c) => c.copy(mode = beta.name)}
        .children(opt[Int]("b-value").required()
            .action{ (p, c) => c.copy(b = p)})
