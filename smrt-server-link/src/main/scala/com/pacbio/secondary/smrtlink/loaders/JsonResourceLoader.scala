...
/**
  ...
  */
trait JsonResourceLoader[T]
    extends ResourceLoaderBase[T]
  ...
  def loadResourcesForSbt: Seq[T] = {
    ...
    val pts = files
      ...
      .map(x => loadItemFromResourceDir(s"$ROOT_DIR_PREFIX/$x"))
  ...
  def loadResourceFromJar: Seq[String] = {
    ...
    val rs = ResourceList
      ...
      .toList
    logger.info(
      s"Resources from jar found ${rs.length} from $ROOT_DIR_PREFIX $px")
    rs
  }
  ...
  def loadResources: Seq[T] = {
    ...
    if (sbtResources.toOption.exists(_.isEmpty) && jarResources.toOption
          .exists(_.isEmpty)) {
      logger.info(
        "Found empty resource list on both SBT and JAR resource paths.")
      return Nil
    }
