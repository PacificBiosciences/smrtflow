...
case class TechSupportFileBundlerOptions(rootUserData: Path,
                                         output: Path,
                                         user: String,
...
object TechSupportFileBundler
  ...
  def getDefault(sx: String): Option[String] =
    Try { conf.getString(sx) }.toOption
  ...
  val parser =
    new OptionParser[TechSupportFileBundlerOptions]("techsupport-bundler") {

      head(DESCRIPTION)

      arg[File]("userdata")
        .action { (x, c) =>
          c.copy(rootUserData = x.toPath)
        }
      ...
      opt[String]("output")
        .action { (x, c) =>
          c.copy(output = Paths.get(x).toAbsolutePath)
        }
        .validate(validateDoesNotExist)
        .text(
          s"Output TechSupport bundle output (tar.gz) file. Default '${defaults.output.toAbsolutePath}'")
      ...
      opt[String]("smrtlink-id")
        ...
        .action { (x, c) =>
          c.copy(smrtLinkSystemId = Some(UUID.fromString(x)))
        }
        .text(overrideMessage("SMRT Link System Id"))

      opt[String]("smrtlink-version")
        .action { (x, c) =>
          c.copy(smrtLinkVersion = Some(x))
        }
  ...
  def hasRequired(fx: (Path => Boolean))(path: Path) = {
    if (Files.exists(path: Path) && fx(path)) Success(path)
      ...
      Failure(throw new IOException(s"Unable to find required resource $path"))
  }
  ...
  def validateDoesNotExist(sx: String): Either[String, Unit] = {
    ...
    if (Files.exists(px))
      Left(s"File already exists. Please move or rename file $px")
    else Right(Unit)
  }
  ...
  private def getSmrtLinkVersionFromConfig(
      smrtLinkSystemConfig: RootSmrtflowConfig): Option[String] = {
  ...
  def getRequired[T](field: String, opt: Option[T]): Try[T] = {
    opt
      .map(v => Success(v))
      .getOrElse(Failure(throw new Exception(
        s"Unable to get required value, $field, from config")))
  }
  ...
  override def runTool(c: TechSupportFileBundlerOptions): Try[String] = {
    ...
    for {
      systemId <- getRequired[UUID]("SMRT Link System Id",
                                    getOr[UUID](c.smrtLinkSystemId,
                                                configPath,
                                                getSmrtLinkIdFromConfigFile))
      systemVersion <- getRequired[String](
        "SMRT Link System Version",
        getOr[String](c.smrtLinkVersion,
                      configPath,
                      getSmrtLinkVersionFromConfigFile))
      dnsName <- getRequired[String](
        "SMRT Link DNS Name",
        getOr[String](c.dnsName, configPath, getDnsNameFromConfigFile))
      tgzPath <- Try {
        TechSupportUtils.writeSmrtLinkSystemStatusTgz(systemId,
                                                      px,
                                                      c.output,
                                                      c.user,
                                                      Some(systemVersion),
