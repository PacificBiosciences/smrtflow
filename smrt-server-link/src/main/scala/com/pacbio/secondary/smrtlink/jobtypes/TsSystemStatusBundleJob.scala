...
import com.pacbio.secondary.smrtlink.analysis.jobtypes.{TsSystemStatusBundleOptions => OldTsSystemStatusBundleOptions}
...
class TsSystemStatusBundleJob(opts: TsSystemStatusBundleJobOptions) extends ServiceCoreJob(opts){
  ...
  override def run(resources: JobResourceBase, resultsWriter: JobResultWriter, dao: JobsDao, config: SystemJobConfig): Either[ResultFailed, PacBioDataStore] = {
    ...
    val manifest = TsSystemStatusManifest(manifestId, BundleTypes.SYSTEM_STATUS, 1, JodaDateTime.now(), config.smrtLinkSystemId,
      Some(config.host), config.smrtLinkVersion, opts.user, Some(opts.comment))
