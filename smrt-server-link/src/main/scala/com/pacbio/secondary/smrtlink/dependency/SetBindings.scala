...
trait SetBinding[T] {
  // scalastyle: on
  final private[dependency] val bindings: mutable.HashSet[Singleton[_ <: T]] =
    new mutable.HashSet[Singleton[_ <: T]]()

  final private[dependency] def registerBinding(obj: Singleton[_ <: T]): Unit =
    bindings.add(obj)

  final private[dependency] def getAllBindings: Set[T] =
    bindings.map(_()).toSet
}
