...
object DataSetValidators {
  ...
  /**
    ...
    * @return
    */
  def validateDataSet(dst: DataSetMetaType,
                      path: Path): Either[InvalidDataSetError, Path] = {

    def vx[T <: DataSetType](
        f: Path => T): Path => Either[InvalidDataSetError, Path] = { px =>
      Try { DataSetValidator.validate(f(px), px.getParent) } match {
        case Success(_) => Right(px)
        case Failure(ex) =>
          Left(
            InvalidDataSetError(
              s"Failed to load DataSet $px ${ex.getMessage}"))
      }
    }
    ...
    val validateHdfSubreadSet =
      vx[HdfSubreadSet](DataSetLoader.loadHdfSubreadSet)
    ...
    val validateGmapReferenceSet =
      vx[GmapReferenceSet](DataSetLoader.loadGmapReferenceSet)
    val validateConsensusReadSet =
      vx[ConsensusReadSet](DataSetLoader.loadConsensusReadSet)
    val validateConsensusAlignmentSet =
      vx[ConsensusAlignmentSet](DataSetLoader.loadConsensusAlignmentSet)
    ...
    if (Files.exists(path)) {
      dst match {
        ...
        case DataSetMetaTypes.AlignmentCCS =>
          validateConsensusAlignmentSet(path)
        case x =>
