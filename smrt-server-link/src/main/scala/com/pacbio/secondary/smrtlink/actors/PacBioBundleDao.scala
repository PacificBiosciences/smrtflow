...
class PacBioBundleDao(bundles: Seq[PacBioDataBundleIO] = Seq.empty[PacBioDataBundleIO]) extends PacBioDataBundleConstants{
  ...
  def getBundle(bundleType: String, version: String): Option[PacBioDataBundle] =
    PacBioBundleUtils.getBundlesByType(getBundles, bundleType).find(_.version == version)
  ...
  /**
    ...
    * @return
    */
  def activateBundle(rootBundleDir: Path, bundleType: String, bundleVersion: String): Try[PacBioDataBundleIO] = {
    val errorMessage = s"Unable to find $bundleType with version $bundleVersion"
