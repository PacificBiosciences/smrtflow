...
class RunServiceActor(runDao: RunDao) extends PacBioActor {
  ...
  def receive: Receive = {
    ...
    case GetCollections(runId) =>
      pipeWith(runDao.getCollectionMetadatas(runId))
    case GetCollection(runId, id) =>
      pipeWith(runDao.getCollectionMetadata(runId, id))
  }
}
...
trait RunServiceActorRefProvider {
  ...
  final val runServiceActorRef: Singleton[ActorRef] =
    Singleton(
      () =>
        actorRefFactory().actorOf(Props(classOf[RunServiceActor], runDao()),
                                  "RunServiceActor"))
}
...
/**
  ...
  */
trait RunServiceActorProvider { this: RunDaoProvider =>
