...
case class ImportBarcodeFastaJobOptions(path: Path, name: Option[String], description: Option[String],
  ...
  override def validate(dao: JobsDao, config: SystemJobConfig):Option[InvalidJobOptionError] = {
    if (Files.exists(path)) None else Some(InvalidJobOptionError(s"Unable to find $path"))
  }
}
...
class ImportBarcodeFastaJob(opts: ImportBarcodeFastaJobOptions) extends ServiceCoreJob(opts){
  ...
  override def run(resources: JobResourceBase, resultsWriter: JobResultWriter, dao: JobsDao, config: SystemJobConfig): Either[ResultFailed, PacBioDataStore] = {
    ...
    val oldOpts = ConvertImportFastaBarcodesOptions(opts.path.toAbsolutePath.toString, name, projectId)
