...
class EngineMultiJobManagerActor(dao: JobsDao, resolver: JobResourceResolver, config:SystemJobConfig) extends Actor with ActorLogging{
  ...
  def checkForWorkById(ix: IdAble) = {
    dao.getMultiJobById(ix)
        .map(engineJob => runner.runWorkflow(engineJob))
        .onFailure { case ex => log.error(s"Failed to check for Multi-Job ${ix.toIdString} ${ex.getMessage}") }
  }

  def checkForWork(): Unit = {
    dao.getEngineMultiJobs(includeInactive = true)
