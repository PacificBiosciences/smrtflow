...
case class MultiAnalysisJobOptions(jobs: Seq[DeferredJob],
                                   ...
                                   projectId: Option[Int] = Some(JobConstants.GENERAL_PROJECT_ID)) extends ServiceMultiJobOptions {
...
class MultiAnalysisJob(opts:MultiAnalysisJobOptions) extends ServiceCoreJob(opts) with ServiceMultiJobModel with DaoFutureUtils with LazyLogging {
  type Out = Int
  ...
  private def resolveEntryPoints(dao: JobsDao, entryPoints: Seq[DeferredEntryPoint]): Future[Seq[(DeferredEntryPoint, DataSetMetaDataSet)]] = {
    Future.sequence(entryPoints.map(ep => dao.getDataSetById(ep.uuid).map(d => (ep, d))))
  }

  private def toPbsmrtpipeOptions(job: DeferredJob, entryPoints: Seq[BoundServiceEntryPoint]): PbsmrtpipeJobOptions = {
    PbsmrtpipeJobOptions(job.name, job.description, job.pipelineId, entryPoints, job.taskOptions, job.workflowOptions, job.projectId)
  }

  private def createPbsmrtpipeJob(dao: JobsDao, entryPoints: Seq[BoundServiceEntryPoint], job: DeferredJob, parentJobId: Int, smrtLinkVersion: Option[String]): Future[EngineJob] = {
    dao.createCoreJob(
      UUID.randomUUID(),
      job.name.getOrElse("Job-Name"),
      job.description.getOrElse("Job-Description"),
      JobTypeIds.PBSMRTPIPE,
      job.entryPoints.map(e => EngineJobEntryPointRecord(e.uuid, e.fileTypeId)),
  ...
  /**
    ...
    *
    */
  private def runJob(dao: JobsDao, job: DeferredJob, jobId: Option[Int], parentJobId: Int, smrtLinkVersion: Option[String]): Future[EngineJob] = {
    for {
      resolvedEntryPoints <- resolveEntryPoints(dao, job.entryPoints)
      engineJobEntryPoints <- Future.successful(resolvedEntryPoints.map(f => BoundServiceEntryPoint(f._1.entryId, f._1.fileTypeId, f._2.uuid)))
      engineJob <- jobId.map(i => dao.getJobById(i)).getOrElse(createPbsmrtpipeJob(dao, engineJobEntryPoints, job, parentJobId, smrtLinkVersion))
    } yield engineJob
  }
  ...
  private def fetchJobState(dao: JobsDao, jobId: Option[Int]): Future[AnalysisJobStates.JobStates] = {
    jobId.map { i =>
      dao.getJobById(i).map(_.state)}
        .getOrElse(Future.successful(AnalysisJobStates.UNKNOWN))
  }

  private def fetchJobStates(dao: JobsDao, jobIds: Seq[Option[Int]]): Future[Seq[AnalysisJobStates.JobStates]] = {
    val fetcher = fetchJobState(dao, _: Option[Int])
    runFuturesSequentially(jobIds)(fetcher)
  }

  private def reduceJobStates(s1: AnalysisJobStates.JobStates, s2: AnalysisJobStates.JobStates): AnalysisJobStates.JobStates = {
    (s1, s2) match {
  ...
  override def runWorkflow(engineJob: EngineJob, resources: JobResourceBase, resultsWriter: JobResultWriter, dao: JobsDao, config: SystemJobConfig): Future[MessageResponse] = {
    ...
    for {
      ...
      updatedWorkflow <- Future.successful(MultiAnalysisWorkflow(updatedJobIds))
      jobStates <- fetchJobStates(dao, updatedJobIds)
      multiJobState <- Future.successful(jobStates.reduce(reduceJobStates))
      updatedMultiJob <- dao.updateMultiJobState(engineJob.id, multiJobState, updatedWorkflow.toJson.asJsObject, s"Updating state to $multiJobState", None)
    } yield MessageResponse(s"Successfully checked MultiJob ${updatedMultiJob.id} ${jobTypeId.id} in state:${updatedMultiJob.state}")
