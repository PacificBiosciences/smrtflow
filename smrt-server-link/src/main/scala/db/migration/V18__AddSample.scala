  ...
  override def slickMigrate(db: DatabaseDef): Future[Any] = db.run {
    ...
    SimpleDBIO {
      _.connection.prepareCall(
        """
CREATE TABLE IF NOT EXISTS "SAMPLE" ("DETAILS" VARCHAR(254) NOT NULL,"UNIQUE_ID" BLOB PRIMARY KEY NOT NULL,"NAME" VARCHAR(254) NOT NULL,"CREATED_BY" VARCHAR(254) NOT NULL,"CREATED_AT" INTEGER NOT NULL);
        """).execute
    }
  }
}
