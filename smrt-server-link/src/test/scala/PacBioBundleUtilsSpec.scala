...
class PacBioBundleUtilsSpec extends Specification{
  ...
  "Bundle Utils tests" should {
    "Sort bundles by semVer" in {
      ...
      def toBundle(v: String) = PacBioDataBundle("a", v, JodaDateTime.now(),
        createdBy, isActive = isActive, description)
    ...
    "Convert manifest.xml to Bundle data model with a description" in {
      val b = PacBioDataBundleIOUtils.parseBundleManifestXml(loader("example-bundles/chemistry-0.1.2/manifest.xml").toFile)
      b.description must beSome
    }
    "Convert manifest.xml to Bundle data model without a description" in {
      val b = PacBioDataBundleIOUtils.parseBundleManifestXml(loader("example-bundles/chemistry-1.2.3/manifest.xml").toFile)
