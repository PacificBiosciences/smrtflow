...
/**
  ...
  */
class PipelineDataStoreViewRulesSpec
  ...
  "Test loading PipelineDataStoreViewRules" should {
    "Sanity test for loading Test file" in {
      ...
      // println(x)
      val pipelineTemplateRules =
        x.parseJson.convertTo[PipelineDataStoreViewRules]
      pipelineTemplateRules.pipelineId must beEqualTo(
        "pbsmrtpipe.pipelines.dev_01")
    }
    "Convert PipelineDataStoreViewRules to Json" in {
      val r1 = DataStoreFileViewRule("source-id",
                                     FileTypes.LOG.fileTypeId,
                                     isHidden = false,
                                     Option("File Name"),
                                     Option("File desc"))
      val r2 = DataStoreFileViewRule("source-id-2",
                                     FileTypes.FASTA.fileTypeId,
                                     isHidden = true,
                                     None,
                                     None)
      val p =
        PipelineDataStoreViewRules("pipeline-id", Seq(r1), "0.1.2-fe4516")
  ...
  "Test PipelineDataStoreViewRulesDao" should {
    val rules = Seq(
      PipelineDataStoreViewRules("dev_01",
                                 Seq.empty[DataStoreFileViewRule],
                                 "4.0"),
      PipelineDataStoreViewRules("dev_01",
                                 Seq.empty[DataStoreFileViewRule],
                                 "5.0"),
      PipelineDataStoreViewRules("dev_01",
                                 Seq.empty[DataStoreFileViewRule],
                                 "5.1"),
      PipelineDataStoreViewRules("dev_02",
                                 Seq.empty[DataStoreFileViewRule],
                                 "5.1")
    )
    val dao = new PipelineDataStoreViewRulesDao(rules)
    "Retrieve rules as list" in {
      dao.getResources.map(r => (r.pipelineId, r.smrtlinkVersion)) must beEqualTo(
        Seq(("dev_01", "5.1"), ("dev_02", "5.1")))
    }
    "Retrieve individual rules" in {
      ...
      dao.getById("dev_01", Some("5.0")).map(_.smrtlinkVersion) must beEqualTo(
        Some("5.0"))
      dao
        .getById("dev_01", Some("5.0.0.SNAPSHOT1234"))
        .map(_.smrtlinkVersion) must beEqualTo(Some("5.0"))
    }
    "Failure modes" in {
      dao.getById("dev_03") must beNone
      dao.getById("dev_01", Some("3.0")) must beNone
      val dao2 = new PipelineDataStoreViewRulesDao(
        Seq.empty[PipelineDataStoreViewRules])
