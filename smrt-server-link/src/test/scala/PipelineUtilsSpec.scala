...
class PipelineUtilsSpec extends Specification {
  ...
  val rsPipelineTemplate = {
    val taskOptions: Seq[PipelineBaseOption] = Seq(
      ...
      PipelineChoiceIntOption("id-e",
                              "name-e",
                              1,
                              "Description-E",
                              Seq(1, 2, 3)),
      PipelineChoiceStrOption("id-f",
                              "name-f",
                              "b",
                              "Description-F",
                              Seq("a", "b", "c")),
      PipelineChoiceDoubleOption("id-g",
                                 "name-g",
                                 0.1,
                                 "Description-G",
                                 Seq(0.01, 0.1, 1.0))
  ...
  val preset1 = {
    ...
    PipelineTemplatePreset("preset-01",
                           rsPipelineTemplate.id,
                           defaultEngineOptions.map(_.asServiceOption),
                           taskOptions)
  }
  ...
  val preset2 = {
    ...
    PipelineTemplatePreset("preset-02",
                           rsPipelineTemplate.id,
                           defaultEngineOptions.map(_.asServiceOption),
                           taskOptions)
  }
  ...
  val preset3 = {
    val taskOptions = Seq()
    PipelineTemplatePreset("preset-03",
                           rsPipelineTemplate.id,
                           defaultEngineOptions.map(_.asServiceOption),
                           taskOptions)
  }
  ...
  val preset4 = {
    ...
    PipelineTemplatePreset("preset-04",
                           rsPipelineTemplate.id,
                           defaultEngineOptions.map(_.asServiceOption),
                           taskOptions)
  }
  ...
  val preset5 = {
    ...
    PipelineTemplatePreset("preset-05",
                           "other_pipeline_id",
                           defaultEngineOptions.map(_.asServiceOption),
                           taskOptions)
  }
  ...
  "Test " should {
    "Simple load/merge Pipeline Presets" in {
      val pipelineTemplate =
        PipelineUtils.updatePipelinePreset(rsPipelineTemplate, Seq(preset1))
    ...
    "Convert from string values" in {
      ...
      px2.map(x => x.asInstanceOf[ServiceTaskStrOption].value) must beEqualTo(
        Some("value-preset-b"))
      val px3 = getOpt(pipelineTemplate, "id-c")
      px3.map(x => x.asInstanceOf[ServiceTaskDoubleOption].value) must beEqualTo(
        Some(9999.99))
      val px4 = getOpt(pipelineTemplate, "id-d")
      px4.map(x => x.asInstanceOf[ServiceTaskBooleanOption].value) must beEqualTo(
        Some(false))
      val px5 = getOpt(pipelineTemplate, "id-e")
      px5.map(x => x.asInstanceOf[ServiceTaskIntOption].value) must beEqualTo(
        Some(2))
      val px6 = getOpt(pipelineTemplate, "id-f")
      px6.map(x => x.asInstanceOf[ServiceTaskStrOption].value) must beEqualTo(
        Some("a"))
      val px7 = getOpt(pipelineTemplate, "id-g")
      px7.map(x => x.asInstanceOf[ServiceTaskDoubleOption].value) must beEqualTo(
        Some(0.01))
    }
