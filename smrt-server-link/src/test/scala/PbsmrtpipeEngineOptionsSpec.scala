...
class PbsmrtpipeEngineOptionsSpec extends Specification {
  ...
  "Simple translating of XML opts to EngineOptions" should {
    "Simple pipeline option of nproc=7 dist=false" in {
      val opts = Seq(
        ...
        ServiceTaskBooleanOption(PbsmrtpipeConstants.DISTRIBUTED_MODE.id,
                                 false,
                                 "desc")
      )
  ...
  "Convert to and from file formats" should {
    ...
    val taskOptions: Seq[ServiceTaskOptionBase] = Seq(
      ...
      PipelineChoiceStrOption("id6",
                              "name6",
                              "A",
                              "Description 6",
                              Seq("A", "B", "C")),
      PipelineChoiceIntOption("id7",
                              "name7",
                              2,
                              "Description 7",
                              Seq(1, 2, 3)),
      PipelineChoiceDoubleOption("id8",
                                 "name8",
                                 0.1,
                                 "Description 8",
                                 Seq(0.01, 0.1, 1.0))
    ...
    "Read presets.json written by installer and validate engine options" in {
      ...
      valsById("pbsmrtpipe.options.chunk_mode")
        .asInstanceOf[ServiceTaskBooleanOption]
        .value must beTrue
      valsById("pbsmrtpipe.options.distributed_mode")
        .asInstanceOf[ServiceTaskBooleanOption]
        .value must beTrue
      valsById("pbsmrtpipe.options.max_total_nproc")
        .asInstanceOf[ServiceTaskIntOption]
        .value must beEqualTo(9999)
      valsById("pbsmrtpipe.options.max_nproc")
        .asInstanceOf[ServiceTaskIntOption]
        .value must beEqualTo(23)
      valsById("pbsmrtpipe.options.max_nchunks")
        .asInstanceOf[ServiceTaskIntOption]
        .value must beEqualTo(23)
      valsById("pbsmrtpipe.options.max_nworkers")
        .asInstanceOf[ServiceTaskIntOption]
        .value must beEqualTo(100)
      valsById("pbsmrtpipe.options.cluster_manager")
        .asInstanceOf[ServiceTaskStrOption]
        .value must beEqualTo(
        "/opt/smrtlink/userdata/generated/config/jms_templates")
      valsById("pbsmrtpipe.options.tmp_dir")
        .asInstanceOf[ServiceTaskStrOption]
        .value must beEqualTo("/opt/smrtlink/userdata/tmp_dir")
      ...
      valsById("pbsmrtpipe.options.exit_on_failure")
        .asInstanceOf[ServiceTaskBooleanOption]
        .value must beFalse
      valsById("pbsmrtpipe.options.debug_mode")
        .asInstanceOf[ServiceTaskBooleanOption]
        .value must beFalse
    }
  }
