...
class SimpleServiceJobType(dbActor: ActorRef, engineManagerActor: ActorRef) extends JobTypeService with LazyLogging {
...
trait SimpleServiceJobTypeProvider {
  ...
  val simpleServiceJobType: Singleton[SimpleServiceJobType] =
    Singleton(() => new SimpleServiceJobType(jobsDaoActor(), engineManagerActor())).bindToSet(JobTypes)
}
