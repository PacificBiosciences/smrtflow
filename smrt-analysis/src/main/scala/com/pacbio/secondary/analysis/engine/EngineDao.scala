...
object EngineDao {
  ...
  trait InMemoryJobEngineDaoComponent extends JobEngineDaoComponent with LazyLogging {
    ...
    private def runnableJobWithIdToEngineJob(runnableJob: RunnableJobWithId, state: AnalysisJobStates.JobStates): EngineJob = {
      ...
      val jsonSettings = "{}"
      EngineJob(runnableJob.id, runnableJob.job.uuid, name, description, createdAt, updatedAt, state, runnableJob.job.jobOptions.toJob.jobTypeId.id, path.toAbsolutePath.toString, jsonSettings, None, None, None)
    }
    ...
    override def updateJobStateByUUID(uuid: UUID, state: AnalysisJobStates.JobStates): Future[String] = Future {
      _engineJobs.get(uuid) match {
        case Some(x) =>
          ...
          val engineJob = EngineJob(x.id, x.uuid, x.name, x.comment, x.createdAt, updatedAt, state, x.jobTypeId, x.path, x.jsonSettings, x.createdBy, None, None)
