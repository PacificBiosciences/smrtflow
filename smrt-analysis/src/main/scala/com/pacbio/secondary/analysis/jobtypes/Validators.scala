...
import com.pacbio.secondary.analysis.jobs.InvalidJobOptionError
...
object Validators {
  ...
  def fileExists(x: String): Option[InvalidJobOptionError] = {
    if (Files.exists(Paths.get(x))) None else Some(InvalidJobOptionError(s"Unable to find $x"))
  }
  ...
  def validateDataSetType(datasetType: String):Option[InvalidJobOptionError] = {
    DataSetMetaTypes.toDataSetType(datasetType) match {
      case Some(x) => None
      case None =>
        val supportedTypes = DataSetMetaTypes.ALL.map(_.toString).reduceLeft((acc, b) => acc + " " + b)
        Some(InvalidJobOptionError(s"Invalid DataSet type $datasetType. Supported Types $supportedTypes"))
    }
  }
