...
object ReportRulesModels {
  ...
  case class AttributeRules(defaultView: String, order: Boolean, rules: List[AttributeRule])
  // Id of the report
  ...
  case class ReportView(inputs: List[String], attributeRules: List[AttributeRule], plotGroupRules: List[PlotGroupRule])
  ...
  def parseXML(f: File): Boolean = {
    ...
    // Attribute Rules
    val attrRules = (root \ "attributesRules" \ "attributeRules" ).map(x => (x \ "@id", x \ "@name"))

    // Table Rules
    val tRules = (root \ "tablesRules" \ "tableRules" ).map(x => (x \ "@id", x \ "@name"))
    val cRules = (root \ "tablesRules" \ "tableRules" \ "columnRules" ).map(x => (x \ "@id", x \ "valueFormat" \ "@type"))

    // Plot Groups Rules
    val pgRules = (root \ "plotGroupsRules" \ "plotGroupRules").map(x => (x \ "@id", x \ "@hidden"))
    true
  }
