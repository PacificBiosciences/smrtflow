...
import com.pacbio.secondary.analysis.pbsmrtpipe.{CommandTemplateJob, CommandTemplate}
...
class PbsmrtpipeCommandTemplateSpec extends Specification{
  ...
  "Simple template " should {
    ...
    "Render simple template" in {
      ...
      val nproc = 12
      val cmd = "ls -ls /tmp"
      val sx = """my-command-runner ${JOB_ID} ${NPROC} ${STDOUT_FILE} ${STDERR_FILE} "${CMD}" """

      val renderCorrect = s"my-command-runner $jobId $nproc $stdout $stderr " + '"' + cmd + "\" "
      val st = new CommandTemplate(sx)
      val commandTemplateJob = CommandTemplateJob(jobId, nproc, Paths.get(stdout), Paths.get(stderr), cmd)
