...
class MockedDbBackUpJob(opts: DbBackUpJobOptions) extends DbBackUpJob(opts: DbBackUpJobOptions) {

  override def runBackUp(output: Path, dbName: String, port: Int, user: String, password: String, stdout: Path, stderr: Path, exe: String = "pg_dumpall"): Try[String] = {
...
class DbBackUpJobSpec extends Specification with SecondaryJobJsonProtocol with LazyLogging{
  ...
  val jobOptions = DbBackUpJobOptions(backUpDir, dbName = "test", dbPort = 1234, dbUser = "test-user", dbPassword = "test-password")
  ...
  "Sanity test for DB BackUp job with Mocked runBackUp cmd" should {
    "Generate a datastore" in {
      ...
      val previousBackUps = (1 until numPreviousBackUps)
          .map(n => s"${jobOptions.baseBackUpName}-file-$n${jobOptions.backUpExt}")
      ...
      val dataStore = FileUtils.readFileToString(dataStorePath.toFile).parseJson.convertTo[PacBioDataStore]
