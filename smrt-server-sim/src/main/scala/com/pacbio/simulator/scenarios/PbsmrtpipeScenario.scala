...
class PbsmrtpipeScenario(host: String, port: Int)
  ...
  val diagnosticJobTests = Seq(
    ...
    dataStore := GetJobDataStore(jobId2),
    ...
    //jobOptions := GetJobOptions(job.mapWith(_.id)),
    ...
    jobReports := GetJobReports(job.mapWith(_.uuid)),
  ...
  val childrenTests = Seq(
    RunDevPipelineAndVerifyChildren(subreadsUuid, numChildren = 384))
