...
/**
  ...
  *
  */
class MultiAnalysisScenario(client: SmrtLinkServiceAccessLayer, testData: PacBioTestResources)
    extends Scenario
        with VarSteps
  ...
  def toJobOptions(subreadset: TestDataResource, uuid: UUID, numJobs: Int): MultiAnalysisJobOptions = {
    val entryPoint = DeferredEntryPoint(DataSetMetaTypes.Subread.fileType.fileTypeId, uuid, "eid_subread")
    ...
    val jobs = (1 to numJobs).map { i =>
      DeferredJob(Seq(entryPoint),
        ...
        taskOptions, taskOptions, Some(s"Sim Scenario MJob Create $i"), None, None)

    }

    MultiAnalysisJobOptions(jobs, Some("Scenario Multi-job"), None, Some(JobConstants.GENERAL_PROJECT_ID))
  }

  def getFileOrFail(testFileId: String): Future[TestDataResource] = {
    testData.files.find(_.id == testFileId).map(f => Future.successful(f))
        .getOrElse(Future.failed(new Exception(s"Unable to find TestFile id $testFileId")))
  }
  ...
  def runSanityTest(subreadsetTestFileId: String, numJobs: Int): Future[Seq[EngineJob]] = {
    for {
      subreadset <- getFileOrFail(subreadsetTestFileId)
      _ <- andLog(s"Loaded TestDataFile $subreadset")
      msg <- client.getStatus
      _ <- andLog(s"Successfully connected to SMRT Link Server: ${client.baseUrl} ${msg.message}")
      ...
      successfulImportJob <- Future.fromTry(client.pollForSuccessfulJob(importJob.id))
      _ <- andLog(s"Successfully imported dataset ${successfulImportJob.id}")
      multiJob <- client.createMultiAnalysisJob(toJobOptions(subreadset, dst.uuid, numJobs))
      _ <- client.updateMultiAnalysisJobToSubmit(multiJob.id)
      successfulMultiJob <- Future.fromTry(client.pollForSuccessfulJob(multiJob.id))
      ...
      _ <- andLog(s"Got children jobs ${jobs.map(_.id)} from Multi-Job ${successfulMultiJob.id}")
    } yield jobs
  }


  case class RunMultiJobAnalysisSanity(subreadsetTestFileId: String, numJobs: Int) extends VarStep[Seq[EngineJob]] {
