...
import com.pacbio.simulator.ICSModel.{
  ICSRun,
  InstrumentState,
  RunRequirements,
  RunResponse
...
object ICSState {
  ...
  final val ALL = Set(Ready,
                      ...
                      Paused,
                      Unknown)
  def fromIndex(ind: Int): Option[State] =
    ALL.map(xx => (xx.ind, xx)).toMap.get(ind)
}
...
/**
  ...
  */
class InstrumentControlClient(val baseUrl: URL)(
    implicit actorSystem: ActorSystem)
    extends ICSJsonProtocol {
  ...
  val mapToJson: HttpResponse => HttpResponse = { response =>
    response.withEntity(
      HttpEntity(ContentTypes.`application/json`, response.entity.data))
  }

  def runPostPipeline = sendReceive ~> mapToJson

  def getInstrumentStatePipeline: HttpRequest => Future[InstrumentState] =
    sendReceive ~> unmarshal[InstrumentState]

  def getRunRequirementsPipeline: HttpRequest => Future[RunRequirements] =
    sendReceive ~> unmarshal[RunRequirements]
  ...
  def runStatusPipeline: HttpRequest => Future[RunResponse] =
    sendReceive ~> unmarshal[RunResponse]

  // POST /run  NOTE : cannot retrieve object, json contains an element with name "type"
  def postRun(icsRun: ICSRun) = runPostPipeline {
    Post(toUrl(RUN_URI), icsRun) ~> addHeader(
  ...
  def postLoadInventory = runPostPipeline {
    Post(toUrl(LOAD_INVENTORY_URI)) ~> addHeader(
      `Content-Type`(`application/json`))
  }
  ...
  def getInstrumentState = getInstrumentStatePipeline {
    Get(toUrl(INSTRUMENT_STATE_URI)) ~> addHeader(
      `Content-Type`(`application/json`))
  }
}
