...
trait ConditionalSteps { this: Scenario with VarSteps =>

  implicit class ConditionalStep(s: Step) {
    def IF(cond: Var[Boolean]): Step = IfStep(cond, s)
    def SHOULD_FAIL: Step = ExpectFailStep(s)
    def SHOULD_RAISE[T <: Throwable: ClassTag](clazz: Class[T]): Step =
      ExpectErrorStep[T](s, clazz)
  }
  ...
  case class IfStep[T](v: Var[Boolean], step: Step) extends Step {
    ...
    override def run: Future[Result] =
      if (v.get) step.run else Future { SUPPRESSED }
  }
  ...
  case class ExpectErrorStep[T <: Throwable: ClassTag](step: Step,
                                                       clazz: Class[T])
      extends Step {
    ...
    override def run: Future[Result] =
      step.run
        ...
        .recoverWith {
          case ex if classTag[T].runtimeClass.isInstance(ex) =>
            Future { SUCCEEDED }
        }
  }
